...
let root_render: Function
let middlewares: MiddleWare[] = []

async function call_middleware(index: number, ctx, route_handler) {
    let nextFn
    try {
        if (index >= 0) {
...
let app = {
    ...
    htmlErrorHandler: (ctx: Ctx, message) => {
        // user error, generated by ctx.err()
...
async function append_route_from_file(file) {
